...
def rotate_model(model, args):
    ...
    if args.checkpoint_local_path is not None:
        R_cpk = args.checkpoint_local_path
...
def online_rotate(module, inp):
    x = torch.nn.functional.linear(inp[0], module.R1)
    return (x,) + inp[1:]


def register_online_rotation(module, R1: torch.Tensor):
    assert not hasattr(module, "R1")
    module.register_buffer(
        "R1", R1.T.to(module.weight.data)
